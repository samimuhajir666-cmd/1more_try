...
def transcribe_deepgram(audio_bytes, last_speaker=None):
    ...
    if r.status_code != 200:
        return {"text": "", "confidence": 0.0, "speaker_count": 0, "success": False, "error": f"Deepgram {r.status_code}"}
    data = r.json()
    text, conf, spk_count = pick_better_speaker(data, last_speaker=last_speaker)
    text = postprocess_text(text)
    if not text or len(text.strip()) < 2:
        return {"text": "[No clear speech detected]", "confidence": 0.0, "speaker_count": spk_count, "success": True, "error": None}
